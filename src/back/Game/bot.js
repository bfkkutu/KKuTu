...
exports.ban = (target, judge, reason, bannedUntil) => {
  let message = new Discord.MessageEmbed()
    ...
    .addFields(
      {
        name: "제재 대상자",
        value: `${target.id.discordify()} (${target.nickname})`,
      },
      { name: "제재 사유", value: reason },
      {
        name: "처리자",
        value: typeof judge === "string" ? judge.discordify() : "알 수 없음",
      },
      {
        name: "처리 결과",
        value: `계정 정지 (${
          bannedUntil == "permanent" ? "영구" : Outer.parseDate(bannedUntil)
        })`,
      }
    );
...
exports.chatban = (target, judge, reason, bannedUntil) => {
  let message = new Discord.MessageEmbed(
    typeof judge === "string" ? judge.discordify() : "알 수 없음"
  )

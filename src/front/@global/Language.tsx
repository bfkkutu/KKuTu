...
const PATTERN_RESOLVER: Table<PatternResolver> = {
  ...
  HUMAN_D: (key, data) => (
    <React.Fragment key={key}>
      {getHumanDigitalSpace(Number(data))}
    </React.Fragment>
  ),
  HUMAN_M: (key, data) => (
    <React.Fragment key={key}>{getHumanMinutes(Number(data))}</React.Fragment>
  ),
  HUMAN_N: (key, data) => (
...
export const getHumanMinutes = (minutes: number) => {
  ...
  if (minutes < 43800)
    return L.get(
      ...
      Math.round((minutes % 1440) / 60)
    );
  return L.get(
    "MINUTES_4",
    Math.floor(minutes / 43800),
    Math.round((minutes % 43800) / 1440)
  );
};

...
DB.ready = function(){
	...
	DB.kkutu_shop_desc.find().on(($docs) => {
		...
		function flush(lang){
			...
			for(let j in $docs) db[$docs[j]._id] = [ $docs[j][`name_${lang}`], $docs[j][`desc_${lang}`] ];
		}
	});
...
function GameClient(id, url){
	...
	const onError = (err) => {
		my.connected = true;
		if(GLOBAL.GAME_SERVER_RETRY > 0) my.tryConnect++;
	...
	const onClose = (code) => {
		...
		if (my.tryConnect <= GLOBAL.GAME_SERVER_RETRY) {
			JLog.info(`Retry connect ..` + (GLOBAL.GAME_SERVER_RETRY > 0 ? `, try: ${my.tryConnect}` : ''));
			setTimeout(() => {
				...
				my.socket.on('open', onOpen);
				my.socket.on('error', onError);
				my.socket.on('close', onClose);
				my.socket.on('message', onMessage);
			}, 5000);
		} else {
	...
	if(!req.query.cp){
		...
		DB.users.findOne([ '_id', id ]).on(($u) => {
			if(!$u) return res.sendStatus(404);
				...
				if($u.newuser) DB.users.update([ '_id', id ]).set([ 'newuser', false ]).on();

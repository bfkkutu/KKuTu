...
const getCFRewards = (word, level, blend, event) => {
    ...
    if (blend) {
        ...
        if (wur >= 0.05) {
            if (wur > 1)
                R.push({ key: "$WPC?", value: Math.floor(wur), rate: 1 });
...
*/
            ...
            if (!$user) return res.send({ message: "FAIL" });
                ...
                if (postM < 0) return res.send({ message: "MONEYFAIL" });
                    ...
                    MainDB.users
                        ...
                        .on(($fin) => {
                            JLog.log(
                                `[CLAN PURCHASED] ${req.body.name} by ${req.body.me}`
                            );
                            MainDB.users
                                .update(["_id", req.body.me])
                                .set(["clan", newClanID])
                            ...
                            MainDB.clans
                                .insert(
                                    ...
                                    [
                                        "users",
                                        JSON.parse(`{"${req.body.me}":2}`),
                                    ],
                                    ["name", req.body.name],
                                    [
                                        "blacklist",
                                        JSON.parse(`{"${req.body.me}":false}`),
                                    ],
                                    ["password", sha256.x2(req.body.password)],
                                    [
                                        "uname",
                                        JSON.parse(
                                            `{"${req.body.me}":"${$user.nickname}"}`
                                        ),
                                    ]
                                )
                                .on();
            ...
            if ($ec.users[req.body.me] == 0)
                ...
                MainDB.users.findOne(["_id", req.body.me]).on(($user) => {
                    if (!$user) return res.send({ message: "FAIL" });
                        ...
                        if (postM < 0)
                            return res.send({ message: "MONEYFAIL" });
                        else {
                            MainDB.users
                                .update(["_id", req.body.me])
                                .set(["money", postM])
    ...
    Server.get("/clan/user", (req, res) => {
        ...
        MainDB.users.findOne(["_id", req.query.id]).on(($user) => {
            if (!$user.clan)
                return res.send({ _id: undefined, name: undefined });
            else {
                MainDB.clans.findOne(["_id", $user.clan]).on(($ec) => {
                    if (!$ec)
                        return res.send({ _id: undefined, name: undefined });
    ...
    Server.get("/inquire", async (req, res) => {
        ...
        for await (let i of asyncGenerator(mine.length)) {
            inquiries.push(
                fs.readFileSync(`./lib/Web/inquire/${mine[i]}`, "utf8")
            );
        }
    ...
    Server.get("/validation/inventory", async (req, res) => {
        const userInventory = (
            await translateToPromise(
                MainDB.users.findOne(["_id", req.query.id])
            )
        ).box;
            ...
            .on(($user) => {
                ...
                MainDB.kkutu_shop
                    ...
                    .on(($item) => {
                        ...
                        $user.money =
                            Number($user.money) +
                            Math.round(0.2 * Number($item.cost));
                        MainDB.users
                            .update(["_id", uid])

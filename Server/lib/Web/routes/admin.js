...
const noticeAdmin = (req, ...args) => {
  req.ip =
    (req.headers["x-forwarded-for"] || req.headers["x-hw-forwarded-for"]) ===
    undefined
      ? "client"
      : req.headers["x-forwarded-for"] || req.headers["x-hw-forwarded-for"];
...
exports.run = (Server, page, Bot) => {
  ...
  Server.get("/gwalli/shop/:key", (req, res) => {
    ...
    const q = req.params.key == "~ALL" ? undefined : ["_id", req.params.key];
    ...
    (async () => {
      await Promise.all(
        list.map(async (item) => {
          ...
          const $doc = await translateToPromise(TABLE.findOne(["_id", item]));
          if (!$doc)
  ...
  Server.post("/gwalli/warn", (req, res) => {
    ...
    MainDB.users.findOne(["_id", id]).on(($user) => {
      ...
      if (warn >= 4) {
        MainDB.users
          .update(["_id", id])
          .set(["warn", 0], ["black", "경고 누적"], ["bandate", 99999999999999])
          .on();

	...
	$(document).ready(function() {
		...
		$("#user-go").on("click", function(b) {
			...
			$.get(`/gwalli/getWarn?id=${$("#warn-id").val()}`, function(b){
				if(!b.message) alert("작업에 실패했습니다.")
				else{
					$("#warn-count").val(Number(b.message))
				}
			})
		}), $("#warn-set").on('click', function(a) {
			if(confirm(`${$("#warn-id").val()}에게 ${$("#warn-count").val()}회의 경고를 설정합니까?`)){
				if($("#warn-count").val() < 0) alert("경고 누적 횟수는 음수일 수 없습니다.")
				else{
					$.post("/gwalli/warn", {
						id: $("#warn-id").val(),
						warn: Number($("#warn-count").val())
					}, function(b){
						if(b == "OK") alert("작업을 완료했습니다.")
						else alert("작업에 실패했습니다.")
					})
				}
			}else alert("작업이 취소되었습니다.")

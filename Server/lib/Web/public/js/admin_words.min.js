...
!(function() {
	...
	function appendFlag(flag){
		let list = $("#wd-flag").empty()
		for(let i=0; i<7; i++){
			let val = i == 0 ? i : Math.pow(2, i-1);
			list.append($("<option>").attr('value', val).attr("selected", flag == val).text(KOR_FLAG[val] || "잘못된 분류"));
		}
		return;
	}
		...
		$("#dbdel-ok").on("click", function(a) {
			...
			/*, $("#db-del").on("click", function(a) {
				...
				$.get("/gwalli/kkutudb/" + $("#db-word").val() + "?lang=" + $("#db-lang").val(), function(a) {
					...
					appendFlag(a.flag), g.forEach(function(a, g) {

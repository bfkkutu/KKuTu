(function() {
	...
	function getAdmin(){
		$.get("/adminList", function(a) {
			return a.admin.indexOf($data.id);
		});
	}
	...
	function badWords(a) {
		//a = a.replace("<", "&lt");
		if (OSV.test(a)){
			return "[홍보 방지] 타서버 언급은 금지입니다.";
		} else if (XSS.test(a)){
			return "[보안] 금지된 문자열을 포함하고 있는 글을 전송하려고 시도하여 차단되었습니다.";
		} else {
			if (BAD.test(a)) {
	...
	$(document).ready(function() {
		...
		function h() {
			ws = new _WebSocket($data.URL), ws.onopen = function(a) {
				...
				rws && rws.close(), stopAllSounds(), alert(b), $.get("/kkutu_notice.html", function(a) {
					loading(a)
				})
		...
		$("#replay-file").on("change", function(a) {
			...
			$rec = !1, $stage.dialog.replayView.attr("disabled", !0), b && (c.readAsText(b), c.onload = function(a) {
				var b, c;
				try {
					c = JSON.parse(a.target.result), d.html(new Date(c.time).toLocaleString()), e.html(c.version), f.empty();
					for (b in c.players) {
						var g, h = c.players[b];

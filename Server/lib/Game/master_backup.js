...
function getClientIp(req, res) {
	var clientIp = req.socket.upgradeReq.connection.remoteAddress;
	if (!clientIp) {
...
function getTempIp($c) {
	var tempIp = $c.socket._socket.remoteAddress;
	if (!tempIp) {
	...
	switch(cmd){
		...
		case "kick": // 유저 킥
			if(temp = DIC[value]){
				...
				//var clientIp = temp.socket.upgradeReq.connection.remoteAddress;
...
function joinNewUser($c, ip, path) {
	...
	fs.appendFileSync(`../IP-Log/Join_Exit.txt`,`\n#Join:[${$c.socket._socket.remoteAddress}|${$c.id}]     (${thisDate})`, 'utf8',(err, ip, path) => { //기록하고
...
KKuTu.onClientClosed = function($c, code, ip, path){
	...
	var thisDate = moment().format("MM월-DD일|HH시-mm분");
	var clientIp = $c.socket._socket.remoteAddress;
	var clientId = $c.id;
	
	logger.info(`Exit #` + $c.id + ` IP: ${$c.socket._socket.remoteAddress}`);
	fs.appendFileSync(`../IP-Log/Join_Exit.txt`,`\n#Exit:[${$c.socket._socket.remoteAddress}|${$c.id}]     (${thisDate})`, 'utf8',(err, ip, path) => { //기록하고
		if (err) return logger.error(`IP를 기록하는 중에 문제가 발생했습니다.   (${err.toString()})`)
	})

...
const getRewards = (
  ...
  /*
	...
	*/
  if (opts.manner) rw.score *= 0.95; // 매너
  if (opts.midmanner) rw.score *= 0.95; // 미들킬 금지
  ...
  if (opts.strict) rw.score *= 1.4; // 깐깐
  if (opts.sami) rw.score *= 1.6; // 3232
  if (opts.no2) rw.score *= 1.6; // 2글자 금지

  if (opts.returns) rw.score *= 0.25; // 리턴
  if (opts.randomturn) rw.score *= 1.4; // 랜덤 턴
  if (opts.noreturn) rw.score *= 1.1; // 도돌이 금지
  if (opts.ignoreinitial) rw.score *= 2.1; // 두음 법칙 파괴
  if (opts.blockWord) rw.score *= 1.0; // 단어 금지
  if (opts.ogow) rw.score *= 1.1; // 한번만
  if (opts.selecttheme) rw.score *= 1.2; // 주제 선택 (한국어 끝말잇기)
  if (opts.bantheme) rw.score *= 1.0; // 주제 금지 (한국어 끝말잇기)
  if (opts.middletoss) rw.score *= 1.2; // 미들 토스
  if (opts.twenty) rw.score *= 0.95; // 20자 제한
  ...
  constructor(socket) {
    ...
    socket.on("message", (msg) => {
      ...
      try {
        ...
        JLog.error(
          `웹소켓을 통해 해석할 수 없는 문자열을 전송 받았습니다.\n전송받은 문자열: ${msg}\nStringDecoder를 이용해 해석한 결과: ${(msg =
            decoder.write(buffer))}`
        );
        try {
          JLog.log("해석한 문자열로 소켓 통신 핸들링을 다시 시도합니다.");
          data = JSON.parse(msg);
        } catch (e) {
          JLog.error(
            "해석한 문자열 또한 올바른 JSON 형태가 아닙니다.\n소켓 통신 핸들링을 포기합니다."
          );
          return;
        }
      }
...
exports.Room = function (room, channel) {
  ...
  my.roundEnd = (data) => {
    ...
    for (let i in my.players) {
      ...
      if (!o) continue;
      if (o.cameWhenGaming && !o.joinWhileGaming) {
        o.cameWhenGaming = false;
        o.joinWhileGaming = false;
        if (o.form == "O") {
          if (!o.admin) {
            o.sendError(428);
            o.leave();
  ...
  my.export = (target, kickVote, spectate) => {
    ...
    if (spectate && my.gaming) {
      ...
      obj.spectate = {};
      for (let i in my.game.seq) {
        if ((o = DIC[my.game.seq[i]])) obj.spectate[o.id] = o.game.score;
      }
    }
    if (my.practice) {

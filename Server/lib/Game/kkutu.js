...
exports.Client = function (socket, profile, sid) {
  ...
  socket.on("message", (msg) => {
    ...
    if (
      JSON.parse(msg).type != "wsrefresh" &&
      JSON.parse(msg).type != "reloadData"
    )
  ...
  my.refresh = () => {
    ...
    if (my.guest) {
      ...
      DB.users.findOne(["_id", my.id]).on(($user) => {
        let first = !$user;
        let ban = first
          ? { isBanned: false, reason: "", bannedAt: "", bannedUntil: "" }
          : $user.ban;
        let chatban = first
          ? { isBanned: false, reason: "", bannedAt: "", bannedUntil: "" }
          : $user.chatban;
        ...
        if (first)
          $user = { nickname: my.profile.title || my.profile.name, money: 0 };
        if (chatban.isBanned) {
          my.noChat = true;
        }
  ...
  my.enter = (room, pass, spectate, joinWhileGaming) => {
    let $room;
    ...
    if (my.place) {
      ...
      if (Cluster.isMaster) {
        ...
        if (!pass && $room) {
          ...
          if (/*!o.admin && */ $room.kicked.includes(my.id)) {

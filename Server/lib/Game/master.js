...
const joinNewUser = ($c, ip, path) => {
    const thisDate = moment().format("MM월-DD일|HH시-mm분");
    ...
    switch (cmd) {
        ...
        case "chatban": {
            ...
            if (DIC[target]) {
                KKuTu.publish("notice", {
                    value:
                        DIC[target].nickname + "님이 채팅 금지 조치되었습니다.",
                });
    ...
    switch (msg.type) {
        ...
        case "room-spectate": {
            if (ROOM[msg.id] && DIC[msg.target]) {
                ...
                JLog.warn(
                    `Wrong room-spectate id=${msg.id}&target=${msg.target}`
                );
            }
            break;
        }
                ...
                .on(($body) => {
                    ...
                    $c.refresh().then((ref) => {
                        if (ref.result == 200) {
                            ...
                            if (
                                ($c.guest &&
                                    GLOBAL.GOOGLE_RECAPTCHA_TO_GUEST) ||
            ...
            Recaptcha.verifyRecaptcha(
                ...
                (success) => {
                    if (success) {
                        ...
                        JLog.warn(
                            `Recaptcha failed from IP ${$c.remoteAddress}`
                        );
...
KKuTu.onClientClosed = ($c, code, ip, path) => {
    ...
    fs.appendFileSync(
        `../IP-Log/Join_Exit.txt`,
        `\n#Exit:[${$c.remoteAddress}|${$c.id}]     (${thisDate})`,
        "utf8",
        (err) => {
            //기록하고
            if (err)

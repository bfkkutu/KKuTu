...
function processAdmin(id, value, requestId) {
  ...
  switch (cmd) {
    ...
    case "ban": {
      ...
      let [target, reason, bannedUntil] = value.split(",");
      let now = moment().format("YYYYMMDDHH");
      let ban = JSON.stringify({
        isBanned: true,
        reason: reason,
        bannedAt: String(now),
      ...
      KKuTu.publish("notice", {
        value: DIC[target].nickname + "님이 차단되었습니다.",
      });
    ...
    case "yellto": {
      const [target, value] = value.split(",");
    ...
    case "ipban": {
      if ((temp = DIC[value])) {
        var clientId = temp.id;
        var clientIp = getClientIp(temp);
        var IpFilters = JSON.parse(
          File.readFileSync("./lib/Web/filters/User.json")
        );
    ...
    case "ipunban": {
      if (DIC[id])
        DIC[id].send("yell", { value: "해당 기능은 현재 지원하지 않습니다." });
      return null;
    }
    case "warn": {
      if (value.match(/,/g).length != 1) return;

      let [target, newWarn] = value.split(",");
  ...
  switch (msg.type) {
	...
	case "room-joinWhileGaming":
      if (ROOM[msg.id] && DIC[msg.target]) {
        ...
        JLog.warn(`Wrong room-joinWhileGaming id=${msg.id}&target=${msg.target}`);
      }
      break;
    case "room-go":
      if (ROOM[msg.id] && DIC[msg.target]) {
...
function joinNewUser($c, ip, path) {
  ...
  fs.appendFileSync(
    `../IP-Log/Join_Exit.txt`,
    `\n#Join:[${$c.remoteAddress}|${$c.id}]     (${thisDate})`,
    "utf8",
    (err, ip, path) => {
      //기록하고
      if (err)

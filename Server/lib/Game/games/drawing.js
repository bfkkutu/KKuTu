...
exports.roundReady = function () {
  ...
  if (my.game.round <= my.round) {
    ...
    getAnswer.call(my, my.game.theme).then(function ($ans) {
      ...
      if ($ans == null) return my.roundEnd(); //$ans = { _id: '단어 수가 부족하여 정상적으로 게임이 진행되지 않습니다.', type: 'INJEONG', mean: '＂1＂', hit: 0, flag: 2, theme: '0' };
      ...
      my.game.painter = my.game.firstWinner
        ? my.game.firstWinner
        : my.game.seq[getRandomIntInclusive(0, my.game.seq.length - 1)];
...
exports.submit = function (client, text) {
  ...
  let play =
    (my.game.seq ? my.game.seq.includes(client.id) : false) || client.robot;
  let gu = my.game.giveup ? my.game.giveup.includes(client.id) : true;

  if (!my.game.winner) return;
  if (
    my.game.winner.indexOf(client.id) == -1 &&
    text == $ans._id &&
    play &&
    !gu
  ) {
  ...
  if (play)
    if (my.game.primary + my.game.giveup.length + 1 >= my.game.seq.length) {
      /*clearTimeout(my.game.hintTimer);
		clearTimeout(my.game.hintTimer2);
		clearTimeout(my.game.hintTimer3);
		clearTimeout(my.game.hintTimer4);*/

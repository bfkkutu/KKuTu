      ...
      function onDB($doc) {
            ...
            "turnEnd",
            {
              ...
              bonus:
                my.game.mission === true
                  ? score - my.getScore(text, t, true)
                  : 0,
            },
            true
          );
          if (my.game.mission === true && my.opts.mission)
            my.game.mission = getMission(my.game.theme);

          setTimeout(my.turnNext, my.game.turnTime / 6);
          if (!client.robot) client.invokeWordPiece(text, 1);
        }
        ...
        if ($doc) {
          if (!my.opts.injeong && $doc.flag & Const.KOR_FLAG.INJEONG) denied();
            ...
            my.opts.strict &&
            (!$doc.type.match(Const.KOR_STRICT) || $doc.flag >= 4)
          )
            denied(406);
...
exports.getScore = function (text, delay, ignoreMission) {
  ...
  if (!ignoreMission && (arr = text.match(new RegExp(my.game.mission, "g")))) {

	...
	function checkTitle(title){
		...
		*/
		if(title == null){
			...
			len = title.length;
			for(i=0; i<len; i++) list.push(getAuto.call(my, title[i], getSubChar.call(my, title[i]), my.game.theme, 1));
...
exports.turnEnd = function(){
	...
	getAuto.call(my, my.game.char, my.game.subChar, my.game.theme, 0).then(function(w){
		my.byMaster('turnEnd', {
			ok: false,
...
exports.submit = function(client, text){
	...
	function onDB($doc){
		...
		function preApproved(){
			...
			if(!my.opts.unknownword && (firstMove || my.opts.manner)) getAuto.call(my, preChar, preSubChar, my.game.theme, 1).then(function(w){
				if(w) approved();
				else {
					my.game.loading = false;
		...
		if($doc){
			...
			if(!my.opts.injeong && ($doc.flag & Const.KOR_FLAG.INJEONG)) denied();
				...
				else if($doc.theme.match(toRegex(my.game.theme)) == null) denied(407);
				else preApproved();
			}
		} else {
...
/*function getAuto(char, subc, type){ //classic.js의 getAuto함수
	/* type
		0 무작위 단어 하나
		1 존재 여부
		2 단어 목록
...
function getAuto(char, subc, theme, type){
	/* type
		0 무작위 단어 하나
		1 존재 여부
		2 단어 목록

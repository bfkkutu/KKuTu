...
$data._soundList = [
		{ key: "ninecircles", value: "/media/kkutu/lovekorea_nine_circles.mp3" }
	];
...
function processAdmin(id, value){
	...
	switch(cmd){
		case "yell":
			...
			return null;
		case "fix": //점검
			var fix = "곧 서버 점검이 있을 예정입니다. 예상 소요 시간: "+value;
		...
		case "someerroroccor": //서버 에러
			var someerroroccor = "서버에 문제가 발생하여 서버가 종료됩니다.";
			KKuTu.publish('yell', { value: someerroroccor });
			return null;
		case "reboot": //서버 재붙
			var reboot = "업데이트 사항 적용을 위해 서버가 재부팅됩니다. 양해 부탁드립니다.";
			KKuTu.publish('yell', { value: reboot });
			return null;
		case "ban": //차단
			var ban = "식별 번호 " + value + "님이 차단되었습니다.";
			KKuTu.publish('yell', { value: ban });
			return null;
		case "ipban": //IP 차단
			var ipban = "식별 번호 " + value + "님이 IP 차단되었습니다.";
			KKuTu.publish('yell', { value: ipban });
			return null;
		case "kill":
			if(temp = DIC[value]){
		...
		case "servermaster":
			...
			if(value=="logonlsh0805020709"){
				...
				if(isAdmin=="bf"){
					...
					var nonecommand = "허가되지 않은 명령어가 사용되었습니다.";
					KKuTu.publish('yell', { value: nonecommand });
				}
			}else{
				isAdmin = "null";
			}
...
Cluster.on('message', function($c, worker, msg, value){
	var temp;
...
function joinNewUser($c, ip, path) {
	...
	logger.info(`New user #` + $c.id + ` IP: ${$c.socket._socket.remoteAddress}`);
	/*var date = moment().format("MM_DD_HH_mm_ss"); //지금 이 시간 (월 일 시 분)
	fs.writeFileSync("../Logs/join/join_"+date+".txt", `New user #` + $c.id + ` IP: ${$c.socket._socket.remoteAddress}`, 'utf8', function(err, ip, path) { //기록하고
});*/
	JLog.info(`New user #` + $c.id + ` IP: ${$c.socket._socket.remoteAddress}`);
}
...
KKuTu.onClientClosed = function($c, code, ip, path){
	...
	logger.info(`Exit #` + $c.id + ` IP: ${$c.socket._socket.remoteAddress}`);
	/*var date = moment().format("MM_DD_HH_mm_ss"); //지금 이 시간 (월 일 시 분)
	fs.writeFileSync("../Logs/exit/exit_"+date+".txt", `Exit #` + $c.id + ` IP: ${$c.socket._socket.remoteAddress}`, 'utf8', function(err, ip, path) { //기록하고
});*/
	JLog.info(`Exit #` + $c.id + ` IP: ${$c.socket._socket.remoteAddress}`);
};
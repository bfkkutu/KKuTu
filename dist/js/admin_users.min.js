	...
	$(document).ready(function() {
		...
		$("#user-go").on("click", function(b) {
			$.get("/gwalli/users?id=" + $("#user-id").val() + "&name=" + $("#user-nick").val(), function(b) {
				...
				b.list.forEach(function(b) {
					$.get(`/newUser?id=${b._id}`, function(n) {
						d.append(c = $("<tr>").attr("id", ["ur", b._id].join("-"))), c.append($("<td>").append(a("ud-" + b._id + "-_id", "g", b._id))).append($("<td>").append(a("ud-" + b._id + "-money", "g", b.money))).append($("<td>").append(a("ud-" + b._id + "-kkutu", "l", JSON.stringify(b.kkutu || {})))).append($("<td>").append(a("ud-" + b._id + "-box", "l", JSON.stringify(b.box || {})))).append($("<td>").append(a("ud-" + b._id + "-equip", "l", JSON.stringify(b.equip || {})))).append($("<td>").append(a("ud-" + b._id + "-nickname", "g", b.nickname))).append($("<td>").append(a("ud-" + b._id + "-server", "t", b.server))).append($("<td>").append(a("ud-" + b._id + "-lastLogin", "t", b.lastLogin))).append($("<td>").append(a("ud-" + b._id + "-ban", "g", JSON.stringify(b.ban)))).append($("<td>").append(a("ud-" + b._id + "-chatban", "g", JSON.stringify(b.chatban)))).append($("<td>").append(a("ud-" + b._id + "-warn", "g", b.warn)))
					})
				})
			})
			...
			if(confirm(`대상: ${$("#monthly-idlist").val()}\n양: ${$("#monthly-ping").val()}핑\n\n계속합니까?`)){
				$.post("/gwalli/monthly", {
					list: $("#monthly-idlist").val().split("\n"),
					ping: Number($("#monthly-ping").val()),
					pw: $("#db-password").val()
				}, function(b){
					if(b.result == "SUCCESS") alert(`대상: ${$("#monthly-idlist").val()}\n양: ${$("#monthly-ping").val()}\n\n월급을 지급했습니다.`)
		...
		}), $("#inquiry-load").on("click", function(b) {
			var date = new Date();
			$.get(`/gwalli/inquire?pw=${$("#m-password").val()}`, async function(b) {
				var c, s, i, o, inquiry, d = $("#inquiry-data").empty();
				for(c in b){
					o = b[c];
					inquiry = b[c].inquiry;
					d.append(s = $("<tr>")), s.append($("<td>").attr("id","inquirer-"+o.inquiry.submitter).text(o.inquiry.submitter)).append($("<td>").attr("id","nickname-"+o.inquiry.submitter).text(o.inquiry.nickname)).append($("<td>").attr("id","title-"+o.inquiry.submitter).text(o.inquiry.title)).append($("<td>").attr("id","date-"+o.inquiry.date).text(o.inquiry.date)).append($("<td>").append($(`<button id="answerbtn-${inquiry.submitter}-${inquiry.date}">`).text("답변하기")))
					document.getElementById(`answerbtn-${inquiry.submitter}-${inquiry.date}`).addEventListener("click", function(){
						$("#inquiry-date").val(inquiry.date);
						$("#inquiry-id").val(inquiry.submitter);
						$("#inquiry-nickname").val(inquiry.nickname);
						$("#inquiry-body").val(inquiry.body);
						$("#answer-date").val(`${date.getFullYear()}_${date.getMonth()+1 < 10 ? "0"+String(date.getMonth()+1) : String(date.getMonth()+1)}_${date.getDate()}_${date.getHours()}_${date.getMinutes()}`);
						$("#answer-id").val("");
						$("#answer-nickname").val("");
						$("#answer-body").val("");
					});
		...
		}), $("#answer-submit").on("click", function(b) {
			$.post(`/gwalli/inquire/answer?pw=${$("#m-password").val()}`, {
				inquiry: {
					id: $("#inquiry-id").val(),
					date: $("#inquiry-date").val()
				},
				answer: {
					answered: true,
					submitter: $("#answer-id").val(),
					nickname: $("#answer-nickname").val(),
					date: $("#answer-date").val(),
					body: $("#answer-body").val()
				}
			}, function(b) {
				if(b.error) alert("답변 등록에 실패했습니다.");
				else alert("답변을 등록했습니다.");
			});
		}), $("#socket-go").on('click', () => {
			ws = new WebSocket(`wss://ws.bfkkutu.kr/g10000/gwalli${$("#db-password").val()}`);
			ws.onopen = (e) => {
				$("#socket-status").text("연결 됨");
				$("#socket-command").attr("disabled", false);
				$("#socket-run").attr("disabled", false);
			}
			ws.onclose = (e) => {
				$("#socket-status").text("연결 안 됨");
				$("#socket-command").attr("disabled", true);
				$("#socket-run").attr("disabled", true);
			}
			ws.onerror = (e) => {
			...
			if(ws && ws.readyState == 1) ws.send(`{"type":"talk","value":"${$("#socket-command").val()}"}`);
			$("#socket-command").val("");
		}), $("#reset-rankpoint").on('click', function(a) {
			$.get(`/gwalli/resetRP?pw=${$("#m-password").val()}`, function(b){
				if(!b.result) alert("작업에 실패했습니다.");
				else{
					alert("작업을 완료했습니다.");
				}
